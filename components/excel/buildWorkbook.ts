/**
 * Excel workbook builder for the A vs B lease comparison.
 *
 * The math is rebuilt as live formulas: assumptions are editable cells,
 * monthly grid + LC + NER + YoC all reference them. The user can change
 * any input in Excel and headline metrics recompute, no round-trip to the
 * web app required.
 *
 ...
 *
 * The Summary sheet up front shows headline metrics for A vs B with
 * formulas referencing the per-scenario sheets.
 *
 * Reference implementation lives in lib/calc.ts; every formula here is a
 * one-to-one translation. The companion smoke test asserts the cached
 * formula values match runScenario() within 1e-6.
 */
